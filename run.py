...
import rembg
import torch
# xatlas is an optional native dependency used only when baking textures.
# Import it lazily or fallback if unavailable so the CLI works without it.
try:
    import xatlas
except Exception:
    xatlas = None
...
parser.add_argument(
    "--preserve-names",
    action="store_true",
    help="If specified, use the original input filename (without extension) as the output subfolder name instead of numeric indices.",
)
parser.add_argument(
    "--overwrite",
    action="store_true",
    help="If specified, overwrite existing outputs in the output directory. Otherwise existing outputs are skipped.",
)
parser.add_argument(
    "--model-save-format",
    default="obj",
...
for i, image_path in enumerate(args.image):
    ...
    # skip if outputs exist and overwrite not requested
    if os.path.exists(out_subdir) and not args.overwrite:
        logging.info(f"Skipping {image_path} because {out_subdir} already exists. Use --overwrite to force re-run.")
        images.append(None)
        folder_names.append(base_name)
        continue

    # ensure output folder exists for this image
    os.makedirs(out_subdir, exist_ok=True)

    if args.no_remove_bg:
        image = np.array(Image.open(image_path).convert("RGB"))
        # save processed input for consistency with earlier behavior
        Image.fromarray(image).save(os.path.join(out_subdir, "input.png"))
    else:
...
for i, image in enumerate(images):
    ...
    # skip images that were intentionally not processed due to existing outputs
    if image is None:
        logging.info(f"Skipping {args.image[i]} (output already exists at {os.path.join(output_dir, folder_names[i])}).")
        continue
    ...
    out_mesh_path = os.path.join(output_dir, folder_names[i], f"mesh.{args.model_save_format}")
    if args.bake_texture:
        out_texture_path = os.path.join(output_dir, folder_names[i], "texture.png")
        ...
        if xatlas is None:
            raise RuntimeError("xatlas is required to export baked textures but it is not installed on this system.")
