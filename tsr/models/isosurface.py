...
try:
    ...
    # Fallback to skimage's marching_cubes if torchmcubes is unavailable.
    # skimage's marching_cubes returns (verts, faces, normals, values).
    # We'll wrap it to provide a compatible signature returning (verts, faces).
    from skimage import measure

    def marching_cubes(volume: torch.Tensor, level: float):
        # Ensure input is on CPU numpy array
        vol_np = volume.detach().cpu().numpy()
        verts, faces, normals, values = measure.marching_cubes(vol_np, level)
